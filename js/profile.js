...
function deleteTeam() {
    $.confirm({
        title: "Confirm!",
        content: "Are you sure you want delete this team?",
        theme: "supervan",
        buttons: {
            Confirm: function () {
                $.ajax({
                    url: url + "/profile/deleteTeam",
                    method: "POST",
                    headers: {
                        "x-access-token": localStorage.getItem("token")
                    },
                    ...
                    success: function (res) {
                        if (res.status !== 200) {
                            $("#errMsgInDelTeam").text(res.message);
                        } else if (res.status === 200) {
                            $("#errMsgInDelTeam").css({ "color": "green" });
                            $("#errMsgInDelTeam").text("Team Deleted Successfully!");
                            setTimeout(function () {
                                window.location.reload(true);
                            }, 1500);
                        }
