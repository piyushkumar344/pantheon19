...
router.post("/eventRegister", verifyToken, async (req, res) => {
  try {
    ...
    await team.eventsRegistered.find(element => {
      return res.json({ status: 200, msg: " Event already registered" });
    });

    team.eventsRegistered.push(event);
    await team.save();

    return res.json({ status: 200, msg: " Successfully Registered" });
  } catch (err) {
    console.log("error in event regisration route ", err);
...
router.post("/eventDeregister", verifyToken, async (req, res) => {
  try {
    ...
    await team.eventsRegistered.find(element => {
      var index = team.eventsRegistered.indexOf(element);
      if (index > -1) {
